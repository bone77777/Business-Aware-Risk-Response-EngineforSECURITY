...
class ResponseAction:
    def __init__(self, name, mitigation_rate, availability_cost, operational_cost):
        self.name = name
        self.mitigation_rate = mitigation_rate  
        self.availability_cost = availability_cost  
...
class BusinessAwareSOC:
    ...
    def multi_objective_optimization(self, threat_prob, degradation, ip):
        ...
        # 2. 対策候補の定義
        actions = [
            ResponseAction("Full_Isolate", 0.99, 1.0, 20),   
            ResponseAction("Throttling", 0.60, 0.2, 5),    
            ResponseAction("Logging_Only", 0.05, 0.0, 1),   
        ]
